...
static unsigned int cpu_diff_us( struct timeval *tv ) {
    int retval;
...
static uttimer_event_Def *heap_remove(uttimer_head_def *uttimer_head, unsigned int uttimer_id) {
    ...
    if(uttimer_id > uttimer_head->cur_timer_num) {
        print_err("timer %d is larger than cur_timer_num %d\n", uttimer_id, uttimer_head->cur_timer_num);
        return NULL;
    }
...
void uttimer_start_withArg(uttimer_head_def *uttimer_head,
                           uttimer_event_Def *uttimer,
                           unsigned int time_invtl,
                           timer_handle_ptr handle,
                           void *arg) {
    if(uttimer_head == NULL || uttimer == NULL || time_invtl == 0 || handle == NULL) {
        print_err("param error\n");
        return;
    }
    ...
    uttimer->timer_handle = handle; //add by liubaolong
...
uttimer_event_Def *uttimer_create_withoutHandle(int is_repeat) {
    uttimer_event_Def *uttimer = NULL;
...
uttimer_head_def *uttimer_init(unsigned int max_timer_num) {
    ...
    if(max_timer_num <= 0) {
        print_err("input param invalid: max_timer_num = %d\n", max_timer_num);
        return NULL;
    }

    uttimer_head = (uttimer_head_def *)malloc(sizeof(uttimer_head_def));
    if(uttimer_head == NULL) {
        print_err("malloc uttimer head fail!\n");
        return NULL;
    }
    ...
    if (NULL == uttimer_head->heap) {
        print_err("malloc heap space fail");
        free(uttimer_head);
        return NULL;
    }
